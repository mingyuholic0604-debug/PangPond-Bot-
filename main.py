...
LEFT = discord.PartialEmoji.from_str(os.getenv("LEFT_EMOJI"))
RIGHT = discord.PartialEmoji.from_str(os.getenv("RIGHT_EMOJI"))
...
def setup_database():
    ...
    try:
        with conn.cursor() as cur:
            ...
            """)
            # 🔹 Ensure card_id is TEXT (fix old DB)
        cur.execute("""
    ALTER TABLE inventory
    ALTER COLUMN card_id TYPE TEXT USING card_id::TEXT;
""")
        # 🔹 Ensure fav_card_id is TEXT
        cur.execute("""
    ALTER TABLE profiles
    ALTER COLUMN fav_card_id TYPE TEXT USING fav_card_id::TEXT;
""")
...
async def balance(interaction: discord.Interaction):
    ...
    try:
        # 🔹 Get balance (now guaranteed to exist)
        data = run_query(
...
async def drop_cmd(
    ...
    try:
        ...
        copies = (copies_res[0] if copies_res else 0) + 1
...
async def daily_cmd(
    ...
    try:
        ...
        run_query(
            "UPDATE users SET boba = boba + %s, cakecoins = cakecoins + %s WHERE user_id = %s",
            (boba, cakecoins, user_id)
        )
...
async def weekly_cmd(
    ...
    try:
        ...
        # 🔥 FIXED: Update instead of insert conflict
        run_query(
            "UPDATE users SET boba = boba + %s, cakecoins = cakecoins + %s WHERE user_id = %s",
            (boba, cakecoins, user_id)
        )
...
async def profile_cmd(
    ...
    try:
        ...
        if fav_card_id:
            fav_card = next(
                (c for c in cards if str(c["id"]).lower() == str(fav_card_id).lower()),
                None
            )

            if fav_card:
                rarity = PANG * int(fav_card.get("rarity", 1))
        ...
        # 🖼️ Card image
        if fav_card and fav_card.get("image"):
