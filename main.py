...
async def drop_cmd(interaction: discord.Interaction):
    ...
    if remaining > 0:
        await interaction.response.send_message(
            f"⏱ Drop cooldown: {remaining//60}m {remaining%60}s left", ephemeral=True
        )
        return
...
async def inventory_cmd(interaction: discord.Interaction):
    ...
    try:
        with conn.cursor() as cur:
            # Group by card_id and count copies
            cur.execute("""
        ...
        def get_embed(page):
            embed = discord.Embed(
                title=f"{interaction.user.name}'s Inventory (Page {page+1}/{total_pages})",
                color=discord.Color.orange()
            )
            ...
            for card_id, name, era, group_name, rarity, copies in data[start:end]:
                ...
                embed.add_field(
                    ...
                    value=f"📀 {era} | 👥 {group_name} | {rarity_str} | Copies: {copies}",
                    inline=False
                )
            return embed
...
async def bake_cmd(interaction: discord.Interaction):
    ...
    if remaining > 0:
        await interaction.response.send_message(
            f"⏱ Bake cooldown: {remaining//60}m {remaining%60}s left", ephemeral=True
        )
        return
