...
class PangpondBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        try:
            ...
            print(f"⚡ Slash commands pushed instantly to Guild: {GUILD_ID}")
        except Exception as e:
...
async def manage_cmd(interaction: discord.Interaction, user: discord.Member, action: app_commands.Choice[str], item_type: app_commands.Choice[str], amount: int = None, card_id: str = None, copies: int = 1):
    ...
    if itype in ["boba", "cakecoins"]:
        if amount is None or amount <= 0:
            return await interaction.followup.send(f"❌ Please provide a valid amount to {act}!")
        
        op = "+" if act == "add" else "-"
        run_query(f"UPDATE users SET {itype} = GREATEST({itype} {op} %s, 0) WHERE user_id=%s", (amount, tid))
        await interaction.followup.send(f"✅ Successfully {act}ed **{amount} {itype}** for {user.mention}.")
        
    elif itype == "card":
        if not card_id:
            return await interaction.followup.send("❌ Please provide a Card ID!")
        
        card = run_query("SELECT card_id, name, era, group_name, rarity FROM cards WHERE card_id=%s", (card_id,), fetchone=True)
        if not card:
            return await interaction.followup.send(f"❌ Card `{card_id}` not found in the registry!")
        ...
        if act == "add":
            ...
            await interaction.followup.send(f"✅ Added {copies}x **{card[1]}** to {user.mention}'s inventory.")
        else:
            ...
            await interaction.followup.send(f"✅ Removed up to {copies}x **{card[1]}** from {user.mention}'s inventory.")
...
async def addcard_cmd(interaction: discord.Interaction, card_id: str, name: str, group: str, era: str, rarity: int, category: str, image_url: str = None, image_file: discord.Attachment = None):
    ...
    await interaction.followup.send(f"✅ Card `{name}` added to system with image!")
