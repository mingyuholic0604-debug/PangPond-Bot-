    ...
    try:
        ...
        async with sess.get(url) as resp:
            ...
            try:
                ...
                img.load()  # 🔥 force load (fixes GitHub/raw issues)
                return img.convert("RGBA")

            except Exception as img_err:
                print(f"❌ Invalid image: {url} -> {img_err}")
                return None
...
async def create_card_grid(image_urls):
    ...
    # 🔥 Load images in parallel (MUCH faster)
    raw_images = await asyncio.gather(
        *(get_image(url) for url in image_urls),
    ...
    for img in raw_images:
        ...
        if isinstance(img, Exception) or img is None:
            img = Image.new("RGBA", (CARD_WIDTH, CARD_HEIGHT), (255, 0, 0, 255))
    ...
    total_width = len(images) * CARD_WIDTH + (len(images) - 1) * PADDING
    total_height = CARD_HEIGHT

    grid = Image.new("RGBA", (total_width, total_height), (0, 0, 0, 0))
...
class DropView(discord.ui.View):
    def __init__(self, user_id, cards):
        ...
        self.message = None  # ✅ needed for timeout edit
    ...
    async def handle_pick(self, interaction: discord.Interaction, index: int):
        ...
        for i, c in enumerate(self.cards, start=1):
            ...
            rarity_display = PANG * int(crarity)
    ...
    async def pick3(self, interaction: discord.Interaction, button: discord.ui.Button):
...
async def drop_cmd(
    ...
    try:
        # ✅ Safe defer (prevents double-ack errors)
        if not interaction.response.is_done():
            await interaction.response.defer()

        # 🔹 Ensure user (non-blocking)
        await asyncio.to_thread(ensure_user, user_id)

        # 🔹 Cooldown check
        remaining = await asyncio.to_thread(
            get_remaining_cooldown,
            user_id, "drop", DROP_COOLDOWN
        )
        ...
        # 🔹 Fetch regular cards
        regular_cards = await asyncio.to_thread(
            run_query,
            """
        ...
        # 🔹 Fetch active event
        active_event = await asyncio.to_thread(
            run_query,
        ...
        event_cards = []
        if active_event:
            event_cards = await asyncio.to_thread(
                run_query,
                """
                SELECT card_id, name, era, group_name, rarity, image
                FROM spec_cards
        ...
        # 🔹 Set cooldown
        await asyncio.to_thread(set_cooldown, user_id, "drop", now)
        ...
        if enabled:
            await asyncio.to_thread(
                set_reminder,
                user_id, "drop", DROP_COOLDOWN, interaction.channel_id
            )

        # 🔥 IMAGE GRID (safe)
        image_urls = [c[5] for c in cards if c[5]]
        ...
        file = discord.File(grid_buffer, filename="drop.png") if grid_buffer else None
        ...
        embed = discord.Embed(
            title=f"{PANCAKE} Card Drop!",
            description=(
                f"{desc}"
                f"{BALL} **You have 60 seconds to pick a card!**\n"
                f"{PANCAKE} Click a button below to claim."
            ),
            color=discord.Color.orange()
        )
        ...
        embed.set_footer(text=f"{ICE} PangPond drops")

        # 🔹 View (no interaction param ❗)
        view = DropView(user_id, cards)
        ...
        # 🔹 Attach message for timeout system
        view.message = msg

        # 🔹 Log
        await log_action(user_id, "drop", "Generated 3-card drop")
...
async def cooldown_cmd(interaction: discord.Interaction):
    ...
    try:
        ...
        # 🔹 Run DB-related stuff safely in thread (prevents lag)
        await asyncio.to_thread(ensure_user, user_id)
...
async def addcard_cmd(
    ...
    try:
        ...
        if rarity < 1 or rarity > 5:
            return await interaction.followup.send(
                "❌ Rarity must be between 1 and 5.",
                ephemeral=True
            )

        # 🔹 Validate category
        if category.lower() not in ["regular", "spec", "custom"]:
            return await interaction.followup.send(
                "❌ Invalid category. Use: regular / spec / custom",
                ephemeral=True
            )

        # 🔹 Check duplicate card_id
        exists = run_query(
            "SELECT 1 FROM cards WHERE LOWER(card_id)=LOWER(%s)",
            (card_id,),
            fetchone=True
        )
        if exists:
            return await interaction.followup.send(
                f"❌ Card `{card_id}` already exists.",
                ephemeral=True
            )
        ...
        """, (card_id, name, era, group, rarity, category.lower(), image))
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"{STAR} {group}\n"
                f"{rarity_display}\n"
                f"{CHOCOLATE} Category: {category.lower()}"
            ),
            color=discord.Color.green()
        )
...
async def deletecard_cmd(interaction: discord.Interaction, card_id: str):
    ...
    try:
        ...
        card_id_clean = card_id.strip().lower()
...
async def startevent(interaction: discord.Interaction, event: str):
    ...
    try:
        # ❌ already active?
        res = run_query(
            "SELECT event_name FROM active_events WHERE event_name = %s;",
            (event,),
            fetchone=True
        )
        ...
        # ✅ add event
        run_query(
            "INSERT INTO active_events (event_name) VALUES (%s);",
            (event,)
        )
        ...
        run_query(
            "INSERT INTO logs (user_id, command, details) VALUES (%s,%s,%s);",
            (str(interaction.user.id), "startevent", f"Started event {event}")
        )
...
async def endevent(interaction: discord.Interaction, event: str):
    ...
    try:
        ...
        # ✅ delete event
        run_query(
            "DELETE FROM active_events WHERE event_name = %s;",
            (event,)
        )
        ...
        run_query(
            "INSERT INTO logs (user_id, command, details) VALUES (%s,%s,%s);",
            (str(interaction.user.id), "endevent", f"Ended event {event}")
        )
...
async def events_cmd(interaction: discord.Interaction):
    ...
    try:
        events = run_query(
            "SELECT event_name FROM active_events;",
            fetchall=True
        )
            ...
            for user_id, command, end_time, channel_id in reminders:
                try:
                    ...
                    embed = discord.Embed(
                        ...
                        description=f"{BALL} Your `{command}` is ready again!",
                        color=discord.Color.orange()
                    )
...
async def on_ready():
    ...
    if background_task is None:
        background_task = bot.loop.create_task(background_loop())
