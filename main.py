...
BALL = os.getenv("BALL_EMOJI")
BOBA = os.getenv("BOBA_EMOJI")
BUTTON = os.getenv("BUTTON_EMOJI")
CAKE = os.getenv("CAKE_EMOJI")
CHOCOLATE = os.getenv("CHOCOLATE_EMOJI")
CROISSANT = os.getenv("CROISSANT_EMOJI")
ICE = os.getenv("ICE_EMOJI") 
LEFT = os.getenv("LEFT_EMOJI")
PANCAKE = os.getenv("PANCAKE_EMOJI")
PANG = os.getenv("PANG_EMOJI")
PUDDING = os.getenv("PUDDING_EMOJI")
RIGHT = os.getenv("RIGHT_EMOJI")
SPIRAL = os.getenv("SPIRAL_EMOJI")
STAR = os.getenv("STAR_EMOJI")
TEA = os.getenv("TEA_EMOJI")
NUKE = os.getenv("NUKE_EMOJI")

# 🔹 Create connection pool safely
db_pool = SimpleConnectionPool(
    1, 10,
    DATABASE_URL,
    sslmode="require"  # important for Railway/hosting
)
...
def set_cooldown(user_id, command, now):
            run_query("""
...
run_query("ALTER TABLE inventory ALTER COLUMN card_id TYPE TEXT;")

run_query("ALTER TABLE profiles ALTER COLUMN fav_card_id TYPE TEXT;")
...
# 🔹 Enable message content if you ever use prefix commands
...
async def start(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title="🍞 PangPond Bot",
        description="Click the button below to start your journey!",
        color=discord.Color.orange()
    )
...
async def drop_cmd(
    ...
    try:
        ...
        # 🔹 Update reminder preference (if user changed it)
...
async def inventory_cmd(
    ...
    try:
        ...
        def get_embed(page):
            embed = discord.Embed(
                title=f"{NUKE} {target.name}'s Inventory (Page {page+1}/{total_pages})",
                color=discord.Color.orange()
            )
...
@app_commands.describe(
    ...
    type="Choose what to modify",
    amount="Amount (currency) or card ID"
)
...
async def handle_cmd(
    ...
    try:
        ...
        if type_value in ["boba", "cakecoins"]:
            ...
            await interaction.response.send_message(
                f"✅ {action_value.title()}ed {amt} {type_value} for {user.mention}"
            )
        ...
        # 🔹 Card handling
            ...
            if action_value == "add":
                ...
                await interaction.response.send_message(
                    f"✅ Card **{card_data['name']}** added to {user.mention}"
                )

            else:
                run_query(
                    "DELETE FROM inventory WHERE user_id=%s AND card_id=%s",
                    (target_id, card_id)
                )

                await interaction.response.send_message(
                    f"✅ Card ID {card_id} removed from {user.mention}"
                )
    ...
    try:
        ...
        for card_id, amount in inputs:
            ...
            if not owned or len(owned) < amount:
                await interaction.response.send_message(
                    f"❌ Not enough copies of card ID {card_id}.",
                    ephemeral=True
                )
                return
            ...
            summary.append(f"{amount}x {card_info[1]}")
...
async def profile_cmd(
    ...
    try:
        ...
        embed.add_field(name="🍰 Cakecoins", value=str(cakecoins))
        embed.add_field(name="🎴 Cards", value=str(card_count), inline=False)
...
# on_ready
# -------------------------------
started = False  # 👈 add this above your function (global)
