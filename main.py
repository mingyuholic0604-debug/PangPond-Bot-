...
@app_commands.describe(
    card_id="Enter the card ID (e.g. FK.KANT.THK)",
    category="Where the card is stored"
)
@app_commands.choices(category=[
    app_commands.Choice(name="Regular", value="regular"),
    app_commands.Choice(name="Spec (Event)", value="spec"),
...
async def deletecard(
    ...
    table_map = {
        "regular": "regular_cards",
        "spec": "spec_cards",
        "custom": "custom_cards"
    }

    table = table_map[category.value]

    # 🔍 Check if card exists
    card = run_query(
        f"SELECT name FROM {table} WHERE card_id=%s",
        (card_id,),
        fetchone=True
    )
    ...
    if not card:
        return await interaction.followup.send(
            f"❌ Card `{card_id}` not found in {category.name}.",
            ephemeral=True
        )
    ...
    # 🗑 Delete card
    run_query(
        f"DELETE FROM {table} WHERE card_id=%s",
        (card_id,)
    )

    # 🧾 Optional: remove from inventories too
    run_query(
        "DELETE FROM inventory WHERE card_id=%s",
        (card_id,)
    )
    ...
    await log_action(
        str(interaction.user.id),
        "deletecard",
        f"Deleted {name} ({card_id}) from {category.value}"
    )

    # ✅ Success message
    await interaction.followup.send(
        f"🗑️ Deleted **{name}** (`{card_id}`) from **{category.name}**.",
        ephemeral=True
    )
        ...
        try:
            ...
            for user_id, command, end_time, channel_id in reminders:
                try:
                    ...
                    # 🔹 Only send if channel supports messages
                    if isinstance(channel, discord.abc.Messageable):
                        ...
                        embed = discord.Embed(
                            ...
                            description=(
                                f"{SPIRAL} <@{user_id}>\n\n"
                                f"{STAR} Your `{command}` is ready to use again!\n"
                                f"{PANG} Don't miss your next drop!"
                            ),
                            color=discord.Color.orange()
                        )
...
async def on_ready():
    ...
    if reminder_task is None:
        reminder_task = bot.loop.create_task(reminder_loop())
