...
def setup_database():
    ...
    try:
        with conn.cursor() as cur:
            ...
            """)
            # 🔧 Fix missing columns (VERY IMPORTANT)
        cur.execute("""
ALTER TABLE users
...
CREATE TABLE IF NOT EXISTS cards (
    ...
    rarity INT,
    image TEXT,
    category TEXT,      -- regular / custom / special
    event_name TEXT     -- NULL if not event
)
""")
        cur.execute("""
...
# -------------------------------
# 🔥 FIX OLD DATABASE (CRITICAL)
# -------------------------------

            # 🔹 Fix inventory.card_id → TEXT
        cur.execute("""
            ALTER TABLE inventory
            ALTER COLUMN card_id TYPE TEXT USING card_id::TEXT;
            """)

            # 🔹 Fix profiles.fav_card_id → TEXT
        cur.execute("""
...
async def manage_cmd(
    ...
    if not any(role.name.lower() in ["mod", "admin"] for role in interaction.user.roles):
        return await interaction.response.send_message(
