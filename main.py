...
GUILD_ID = int(os.getenv("GUILD_ID", "1475099422315647006"))
...
async def group_autocomplete(interaction: discord.Interaction, current: str):
    results = run_query("SELECT DISTINCT group_name FROM cards WHERE group_name ILIKE %s LIMIT 25", (f"%{current}%",), fetchall=True)
...
async def era_autocomplete(interaction: discord.Interaction, current: str):
    results = run_query("SELECT DISTINCT era FROM cards WHERE era ILIKE %s LIMIT 25", (f"%{current}%",), fetchall=True)
    return [app_commands.Choice(name=r[0], value=r[0]) for r in results]

async def category_autocomplete(interaction: discord.Interaction, current: str):
    return [app_commands.Choice(name=c, value=c) for c in ["regular", "spec", "custom"] if current.lower() in c.lower()]
...
async def create_card_grid(image_urls):
    ...
    for img in raw_images:
        if img is None: img = Image.new("RGBA", (300, 400), (40, 40, 40, 255))
        images.append(img)
    total_width = len(images) * 300 + (len(images)-1) * 10
    grid = Image.new("RGBA", (total_width, 400), (0, 0, 0, 0))
    for i, img in enumerate(images): grid.paste(img, (i * 310, 0))
    buffer = BytesIO()
...
class DropView(discord.ui.View):
    ...
    async def pick(self, interaction: discord.Interaction, index: int):
        ...
        copies = run_query("SELECT COUNT(*) FROM inventory WHERE user_id=%s AND card_id=%s", (self.user_id, c[0]), fetchone=True)[0]
        progress = run_query("SELECT COUNT(DISTINCT card_id), (SELECT COUNT(*) FROM cards WHERE group_name=%s) FROM inventory WHERE user_id=%s AND group_name=%s", (c[3], self.user_id, c[3]), fetchone=True)
        
        desc = ""
        for i, card in enumerate(self.cards):
            desc += f"**#{i+1} • `{card[0]}`**\n{SPIRAL} **{card[1]}** ({card[2]})\n{PANG * int(card[4])}\n"
            if i == index: desc += f"{interaction.user.mention}\nGroup Progress: {progress[0]}/{progress[1]}\nCopies: {copies}\n"
            desc += "\n"
        
        embed = discord.Embed(title="Results of the drop!", description=desc, color=discord.Color.purple())
    ...
    async def on_timeout(self):
        for child in self.children: child.disabled = True
        if self.message: 
            try: await self.message.edit(content="⌛ Drop expired!", view=self)
            except: pass
...
async def drop_cmd(interaction: discord.Interaction, reminder: bool = None):
    ...
    ensure_user(uid)
    cards = run_query("SELECT card_id, name, era, group_name, rarity, image FROM cards", fetchall=True)
    if not cards: return await interaction.followup.send("❌ No cards available.")
    def pick():
        pool = []
    ...
    grid = await create_card_grid([c[5] for c in selection if c[5]])
    desc = ""
    for i, c in enumerate(selection): desc += f"**#{i+1} • `{c[0]}`**\n{SPIRAL} **{c[1]}** ({c[2]})\n{STAR} {c[3]}\n{PANG*int(c[4])}\n\n"
    embed = discord.Embed(title=f"{PANCAKE} Card Drop!", description=desc + f"{BALL} Pick a card!", color=discord.Color.orange())
...
async def inventory(interaction: discord.Interaction, user: discord.Member = None, filter_type: str = None, filter_value: str = None):
    ...
    class InvView(View):
        ...
        def get_embed(self):
            ...
            for c in pages[self.p]: e.add_field(name=f"{c[1]} ({c[0]})", value=f"{SPIRAL} {c[2]} | {STAR} {c[3]}\n{PANG*c[4]} | Copies: {c[5]}", inline=False)
            return e
...
@bot.tree.command(name="manage", description="Admin: Manage data")
async def manage_cmd(interaction: discord.Interaction, user: discord.User, action: str, type: str, amount: int = None, card_id: str = None, quantity: int = 1):
    if interaction.guild_id != GUILD_ID or not interaction.user.guild_permissions.manage_guild: return await interaction.response.send_message("❌ Unauthorized", ephemeral=True)
    tid = str(user.id)
    ensure_user(tid)
    if type in ["boba", "cakecoins"]:
        if amount is None: return await interaction.response.send_message("❌ Amount required", ephemeral=True)
        op = "+" if action == "add" else "-"
        run_query(f"UPDATE users SET {type} = GREATEST({type} {op} %s, 0) WHERE user_id=%s", (amount, tid))
        await interaction.response.send_message(f"✅ {action.title()}ed {amount} {type} for {user.mention}", ephemeral=True)
    elif type == "card":
        if not card_id: return await interaction.response.send_message("❌ card_id required", ephemeral=True)
        card = run_query("SELECT card_id, name, era, group_name, rarity FROM cards WHERE card_id=%s", (card_id,), fetchone=True)
        if not card: return await interaction.response.send_message("❌ Card not found", ephemeral=True)
        if action == "add":
            for _ in range(quantity): run_query("INSERT INTO inventory (user_id, card_id, name, era, group_name, rarity) VALUES (%s,%s,%s,%s,%s,%s)", (tid, card[0], card[1], card[2], card[3], card[4]))
            await interaction.response.send_message(f"✅ Added {quantity}x {card[1]} to {user.mention}", ephemeral=True)
        else:
            run_query("DELETE FROM inventory WHERE id IN (SELECT id FROM inventory WHERE user_id=%s AND card_id=%s LIMIT %s)", (tid, card_id, quantity))
            await interaction.response.send_message(f"✅ Removed {quantity}x {card[1]} from {user.mention}", ephemeral=True)
    await log_action(str(interaction.user.id), "manage", f"{action} {type} for {tid}")
...
async def pay_cmd(interaction: discord.Interaction, user: discord.Member, amount: int, currency: str):
    ...
    if amount <= 0 or sid == rid or cur not in ["boba", "cakecoins"]: return await interaction.response.send_message("❌ Invalid pay.", ephemeral=True)
    ensure_user(sid); ensure_user(rid)
    bal = run_query(f"SELECT {cur} FROM users WHERE user_id=%s", (sid,), fetchone=True)[0]
    if bal < amount: return await interaction.response.send_message("❌ Insufficient balance.", ephemeral=True)
    ...
    await interaction.response.send_message(content=user.mention, embed=e)
...
async def giftcard_cmd(interaction: discord.Interaction, user: discord.User, card1: str, amount1: int, card2: str = None, amount2: int = None, card3: str = None, amount3: int = None, card4: str = None, amount4: int = None, card5: str = None, amount5: int = None):
    ...
    for cid, amt in gifts:
        ...
        if len(owned) < amt: return await interaction.followup.send(f"❌ You do not have enough copies of {cid}.")
...
async def profile_cmd(interaction: discord.Interaction, user: discord.Member = None):
    ...
    if p and p[1]:
        ...
        if f: e.add_field(name=f"{PUDDING} Favourite Card", value=f"**{f[0]}** ({p[1]})\n{SPIRAL} {f[1]} | {STAR} {f[2]}\n{PANG*f[3]}", inline=False)
    await interaction.response.send_message(embed=e)
...
async def addcard_cmd(interaction: discord.Interaction, card_id: str, name: str, group: str, era: str, rarity: int, category: str, image: str):
    if interaction.user.id not in ALLOWED_USERS: return await interaction.response.send_message("❌ Permission denied.", ephemeral=True)
    if rarity < 1 or rarity > 5: return await interaction.response.send_message("❌ Rarity must be 1-5.")
    run_query("INSERT INTO cards (card_id, name, era, group_name, rarity, category, image) VALUES (%s, %s, %s, %s, %s, %s, %s) ON CONFLICT (card_id) DO UPDATE SET name=%s, era=%s, group_name=%s, rarity=%s, category=%s, image=%s", (card_id, name, era, group, rarity, category.lower(), image, name, era, group, rarity, category.lower(), image))
    await log_action(str(interaction.user.id), "addcard", f"{name} ({card_id})")
    await interaction.response.send_message(f"✅ Card `{name}` added to system!", ephemeral=True)
