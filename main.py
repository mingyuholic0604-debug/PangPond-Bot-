...
def check_is_manager(interaction: discord.Interaction):
    """Checks if the user is a mod/admin of the target server or a dev."""
    # Always allow devs
    if interaction.user.id in ALLOWED_USERS:
        return True
    
    # Check guild and role
...
async def sync_prefix(ctx):
    ...
    if ctx.author.id in ALLOWED_USERS or (ctx.guild and ctx.guild.id == GUILD_ID and any(r.id == MOD_ROLE_ID for r in ctx.author.roles)):
        await ctx.send("⏳ Syncing commands to Discord... please wait...")
        try:
            fmt = await bot.tree.sync()
            await ctx.send(f"✅ Success! Synced {len(fmt)} slash commands. (It may take a minute for Discord to show them)")
        except Exception as e:
...
async def manage_cmd(interaction: discord.Interaction, user: discord.Member, action: app_commands.Choice[str], item_type: app_commands.Choice[str], amount: int = None, card_id: str = None, copies: int = 1):
    ...
    if itype in ["boba", "cakecoins"]:
        if amount is None or amount <= 0:
            return await interaction.response.send_message(f"❌ Please provide a valid amount to {act}!", ephemeral=True)
        
        op = "+" if act == "add" else "-"
        run_query(f"UPDATE users SET {itype} = GREATEST({itype} {op} %s, 0) WHERE user_id=%s", (amount, tid))
        await interaction.response.send_message(f"✅ Successfully {act}ed **{amount} {itype}** for {user.mention}.", ephemeral=True)
        
    elif itype == "card":
        if not card_id:
            return await interaction.response.send_message("❌ Please provide a Card ID!", ephemeral=True)
        
        card = run_query("SELECT card_id, name, era, group_name, rarity FROM cards WHERE card_id=%s", (card_id,), fetchone=True)
        if not card:
            return await interaction.response.send_message(f"❌ Card `{card_id}` not found in the registry!", ephemeral=True)
        ...
        if act == "add":
            ...
            await interaction.response.send_message(f"✅ Added {copies}x **{card[1]}** to {user.mention}'s inventory.", ephemeral=True)
        else:
            ...
            await interaction.response.send_message(f"✅ Removed up to {copies}x **{card[1]}** from {user.mention}'s inventory.", ephemeral=True)
...
async def addcard_cmd(interaction: discord.Interaction, card_id: str, name: str, group: str, era: str, rarity: int, category: str, image_url: str = None, image_file: discord.Attachment = None):
    ...
    if not final_image:
        return await interaction.response.send_message("❌ You must provide an image URL or upload an image file!", ephemeral=True)

    run_query("""
        INSERT INTO cards (card_id, name, era, group_name, rarity, category, image) 
        VALUES (%s, %s, %s, %s, %s, %s, %s) 
        ON CONFLICT (card_id) DO UPDATE SET 
        name=%s, era=%s, group_name=%s, rarity=%s, category=%s, image=%s
    """, (card_id, name, era, group, rarity, category.lower(), final_image, name, era, group, rarity, category.lower(), final_image))
    
    await log_action(str(interaction.user.id), "addcard", f"Added: {name} ({card_id})")
    await interaction.response.send_message(f"✅ Card `{name}` added to system with image!", ephemeral=True)
...
@app_commands.describe(file="CSV file with columns: card_id, name, group_name, era, rarity, category, image_url")
async def bulkadd_cmd(interaction: discord.Interaction, file: discord.Attachment):
    ...
    try:
        ...
        for row in reader:
            ...
            cid, name, group, era, rarity, cat, img = row
            run_query("""
                INSERT INTO cards (card_id, name, group_name, era, rarity, category, image) 
                VALUES (%s, %s, %s, %s, %s, %s, %s) 
                ON CONFLICT (card_id) DO UPDATE SET 
                name=%s, group_name=%s, era=%s, rarity=%s, category=%s, image=%s
            """, (cid, name, group, era, int(rarity), cat.lower(), img, name, group, era, int(rarity), cat.lower(), img))
            count += 1
