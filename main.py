...
async def create_card_grid(images: list[str]):
    imgs = []
    ...
    # 🔹 Create canvas (3 cards side by side)
    grid = Image.new("RGBA", (width * 3, height))
...
# 🔹 Build embed
        ...
        for i, c in enumerate(cards, start=1):cid, name, era, group, rarity, _ = crarity_display = PANG * int(rarity)
...
@app_commands.describe(
    card_id="Unique card ID (e.g. FK.KANT.THK)",
    name="Card name",
    group="Group name",
    era="Era",
    rarity="Rarity (1-4)",
    category="regular / spec / custom",
    event="Event name (only for spec cards)"
)
async def addcard(
    interaction: discord.Interaction,
    card_id: str,
    ...
    # ❌ Validate category
    if category not in ["regular", "spec", "custom"]:
        await interaction.followup.send("❌ Invalid category. Use: regular / spec / custom")
        return

    # ❌ Validate rarity
    if rarity < 1 or rarity > 4:
        await interaction.followup.send("❌ Rarity must be between 1 and 4")
        return

    # ❌ Spec cards must have event
    if category == "spec" and not event:
        await interaction.followup.send("❌ You must provide an event name for spec cards")
        return

    # ❌ Non-spec should NOT have event
    if category != "spec" and event:
        await interaction.followup.send("❌ Only spec cards can have events")
        return

    try:
        # 🔍 Check duplicate across ALL tables
        for table in ["regular_cards", "spec_cards", "custom_cards"]:
            cursor.execute(f"SELECT card_id FROM {table} WHERE card_id = %s;", (card_id,))
            if cursor.fetchone():
                await interaction.followup.send(f"❌ Card ID `{card_id}` already exists.")
                return

        # ✅ Insert into correct table
        if category == "regular":
            cursor.execute(
                "INSERT INTO regular_cards VALUES (%s, %s, %s, %s, %s);",
                (card_id, name, group, era, rarity)
            )

        elif category == "custom":
            cursor.execute(
                "INSERT INTO custom_cards VALUES (%s, %s, %s, %s, %s);",
                (card_id, name, group, era, rarity)
            )

        elif category == "spec":
            cursor.execute(
                "INSERT INTO spec_cards VALUES (%s, %s, %s, %s, %s, %s);",
                (card_id, name, group, era, rarity, event)
            )
        ...
        cursor.execute(
            ...
            (interaction.user.id, f"Added card {card_id} ({category})")
        )
        conn.commit()

        # ✅ Success message
        msg = f"✅ Card `{card_id}` added to **{category}**"
        if category == "spec":
            msg += f" (Event: {event})"
        ...
        await interaction.followup.send("❌ Error adding card.")
...
async def on_ready():
    ...
    print("Commands loaded:", len(bot.tree.get_commands()))
